...
export const getCloudwatch = regionalCache((region: string) => new CloudWatch({ region }));
...
export const getLambdaClient = regionalCache((region: string) => new LambdaClient({ region }));

export const getS3 = regionalCache((region: string) => new S3({ region }));

export const getSsmClient = regionalCache((region: string) => new SSMClient({ region }));

export const getDynamoDbClient = regionalCache((region: string) => new DynamoDBClient({ region }));
