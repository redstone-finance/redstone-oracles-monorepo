...
export class CantonScanApiClient {
  ...
  async getAmuletHoldingsSummary(partyId: string) {
    ...
    const snapshot = await this.postWithProxy<AcsSnapshotTimestampResponse>(snapshotUrl);

    const result = await this.postWithProxy<HoldingsSummaryResponse>(
  ...
  private async findLatestMigrationId(scanApiUrl: string): Promise<number> {
    const result = await this.postWithProxy<DsoSequencersResponse>(`${scanApiUrl}/dso-sequencers`);
