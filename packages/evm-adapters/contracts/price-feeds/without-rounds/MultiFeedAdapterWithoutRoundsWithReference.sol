...
abstract contract MultiFeedAdapterWithoutRoundsWithReference is IMultiFeedAdapter {
  ...
  uint32 internal constant DEFAULT_GET_LAST_UPDATE_DETAILS_GAS_LIMIT = 2_000_000;
   ...
   * @return maxAllowedDeviation The value deviation between adapters (in basis points) that triggers a switch
   ...
   */
  function getReferenceSwitchCriteria(bytes32 dataFeedId) public view virtual returns (uint256 maxAllowedDeviation, uint256 maxDataAgeInSeconds);
  ...
  function getGasLimitForGetLastUpdateDetailsCall() public pure virtual returns (uint32) {
    return DEFAULT_GET_LAST_UPDATE_DETAILS_GAS_LIMIT;
  }
  ...
  function getLastUpdateDetails(bytes32 dataFeedId) public view returns (
    ...
    (uint256 maxAllowedDeviation, uint256 maxDataAgeInSeconds) = getReferenceSwitchCriteria(dataFeedId);
    uint256 deviation = calculateDeviationSafe(mainValue, refValue);
    ...
    if (deviation > maxAllowedDeviation && isRefDataFresh) {
  ...
  /// @dev Calculates deviation in basis points
  function calculateDeviationSafe(uint256 value1, uint256 value2) public pure returns (uint256) {
  ...
  function _safeGetLastUpdateDetails(IMultiFeedAdapter adapter, bytes32 dataFeedId) internal view returns (bool ok, uint256 lastDataTimestamp, uint256 lastBlockTimestamp, uint256 lastValue) {
    try adapter.getLastUpdateDetails{gas: getGasLimitForGetLastUpdateDetailsCall()}(dataFeedId) returns ( uint256 dataTs, uint256 blockTs, uint256 value) {
      return (true, dataTs, blockTs, value);
    } catch {
      return (false, 0, 0, 0);
    }
  }
