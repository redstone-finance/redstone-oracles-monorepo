...
import { Prices } from "../_autogenerated";
import { PricesConfigurables } from "../_autogenerated/Prices";
import { FuelPricesContract, FuelPricesContractConnector } from "./FuelPricesContractConnector";

const PRICES_CONTRACT_BINARY = "../_autogenerated/prices.bin";
...
export class FuelPricesContractDeployer extends FuelPricesContractConnector {
  ...
  private makeAllowedSigners() {
    ...
    return [
      ...
      .slice(0, 6)
      .map((signer) => toHex(signer, 32)) as [string, string, string, string, string, string];
  }
  ...
  private getStorageSlots() {
    ...
    if (RedstoneCommon.isDefined(this.parameters.fakeTimestamp)) {
      storageSlots.push({
        ...
        value: toHex(this.parameters.fakeTimestamp),
      });
    }
