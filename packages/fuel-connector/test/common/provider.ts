...
export const IS_CI = RedstoneCommon.getFromEnv("IS_CI", z.boolean().default(false));
...
export const provider = async (isLocal = IS_LOCAL) =>
  isLocal ? undefined : await Provider.create(IS_MAINNET ? MAINNET_NODE_URL : TESTNET_NODE_URL);

export async function getWallet(provider?: Provider) {
  const privateKey = RedstoneCommon.getFromEnv("PRIVATE_KEY", z.string().optional());
  if (!privateKey) {
    throw new Error("Non-empty PRIVATE_KEY must be defined in env!");
  }

  return Wallet.fromPrivateKey(privateKey, provider ?? (await Provider.create(LOCAL_NODE_URL)));
}
