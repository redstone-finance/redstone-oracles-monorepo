...
) => Promise<MovementTransactionData>;

export type MovementTransactionData = InputGenerateTransactionPayloadData & {
  identifier: string;
...
export class MovementTxDeliveryMan {
  ...
  async sendTransaction(data: MovementTransactionData) {
  ...
  private async performIteration(
    data: MovementTransactionData,
    iterationIndex: number
  ) {
    return await new Promise<string>((resolve, reject) => {
      this.logger.debug(`Enqueuing #${iterationIndex} ${data.identifier}`);
      const wasEnqueued = MovementTxDeliveryMan.getQueue(this.account).enqueue(
        data.identifier,
        async () => {
          try {
  ...
  private async tryCallFunction(
    ...
    const options = await MovementOptionsContractUtil.prepareTransactionOptions(
      this.client,
  ...
  private async processTransactionError(e: unknown) {
    ...
    if ("transaction" in (e as Error)) {
      ...
      this.logger.error(
        ...
        MovementTxDeliveryMan.extractSimpleFields(fail.transaction)
      );

      didLog = true;

      this.setNextSequenceNumber(fail.transaction.sequence_number);
    } else if (MovementTxDeliveryMan.isSequenceNumberTooOldError(e)) {
      await this.refreshSequenceNumber();
    }
