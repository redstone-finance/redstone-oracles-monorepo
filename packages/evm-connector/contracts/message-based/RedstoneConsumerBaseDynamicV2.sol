...
abstract contract RedstoneConsumerBaseDynamicV2 {
  ...
  function getDynamicOracleValueFromTxMsg(bytes32 symbol) internal view returns (bytes memory) {
    bytes32[] memory symbols = new bytes32[](1);
    symbols[0] = symbol;
    return getDynamicOracleValuesFromTxMsg(symbols)[0];
  }

  function getDynamicOracleValuesFromTxMsg(bytes32[] memory symbols)
    internal
    view
  ...
  // Extracts data and updates helpful arrays
  // Returns total data package byte size
  function _extractDataPackage(
    ...
    // Updating helpful arrays
    {
      ...
      bytes4 dataPointValueByteSize;
      bytes dataPointValue;
      for (
        ...
        assembly {
          ...
          dataPointValueByteSize := calldataload(add(dataPointCalldataOffset, DP_SYMBOL_BS)) // TODO: improve
        }

        // Allocating memory
        dataPointValue = new bytes(dataPointValueByteSize);
        assembly {
          // Extracting dynamic value from calldata
          // TODO: improve
          dataPointValue := calldatacopy(valueStart, dataPointValueByteSize)
        }
