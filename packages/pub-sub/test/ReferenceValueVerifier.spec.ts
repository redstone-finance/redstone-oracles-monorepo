...
function makeSigned(signer: MockSigner, dataPackageId: string, tsMs: number, value: number) {
...
describe("ReferenceValueVerifier", () => {
  ...
  it("returns package immediately when signer is a reference signer", () => {
    const verifier = new ReferenceValueVerifier(new Set([MOCK_SIGNER_1.address]));
    const s = makeSigned(MOCK_SIGNER_1, "BTC", 1730000000000, 100);
  ...
  it("passes when not enough reference values (minReferenceValues > found)", () => {
    const verifier = new ReferenceValueVerifier(
      new Set([MOCK_SIGNER_1.address, MOCK_SIGNER_2.address]),
      1,
      3,
      2
    );
    ...
    verifier.registerDataPackage(makeSigned(MOCK_SIGNER_1, "ETH", ts, 200));

    const tested = makeSigned(MOCK_SIGNER_3, "ETH", ts, 205);
  ...
  it("rejects when enough reference values and deviation >= threshold", () => {
    const verifier = new ReferenceValueVerifier(
      new Set([MOCK_SIGNER_1.address, MOCK_SIGNER_2.address, MOCK_SIGNER_3.address]),
      1,
      3,
      2
    );
    ...
    verifier.registerDataPackage(makeSigned(MOCK_SIGNER_1, "BTC", ts, 100));
    verifier.registerDataPackage(makeSigned(MOCK_SIGNER_2, "BTC", ts, 101));
    verifier.registerDataPackage(makeSigned(MOCK_SIGNER_3, "BTC", ts, 102));

    const tested = makeSigned(UNAUTHORIZED_MOCK_SIGNER, "BTC", ts, 103); // ~1.98% from median 101
    const result = verifier.verifyDataPackage(tested);
  ...
  it("passes when deviation < threshold, with enough references", () => {
    ...
    verifier.registerDataPackage(makeSigned(MOCK_SIGNER_1, "BTC", ts, 100));
    verifier.registerDataPackage(makeSigned(MOCK_SIGNER_2, "BTC", ts, 102));

    const tested = makeSigned(UNAUTHORIZED_MOCK_SIGNER, "BTC", ts, 101.5); // ~0.5% from median 101
    const result = verifier.verifyDataPackage(tested);
  ...
  it("uses values only from reference signers and only for matching dataPackageId", () => {
    ...
    verifier.registerDataPackage(makeSigned(MOCK_SIGNER_1, "SOL", ts, 50));
    verifier.registerDataPackage(makeSigned(MOCK_SIGNER_2, "SOL", ts, 52));
    // non-reference signer (ignored as reference)
    verifier.registerDataPackage(makeSigned(UNAUTHORIZED_MOCK_SIGNER, "SOL", ts, 10));
    // different id (ignored)
    verifier.registerDataPackage(makeSigned(MOCK_SIGNER_3, "BTC", ts, 9999));

    const tested = makeSigned(UNAUTHORIZED_MOCK_SIGNER, "SOL", ts, 51.2); // median 51, ~0.392%
    const result = verifier.verifyDataPackage(tested);
    expect(result).toBe(tested);
    expect(mockError).not.toHaveBeenCalled();
  });
  ...
  it("respects maxDelayInSeconds by using entries from previous seconds", () => {
    ...
    verifier.registerDataPackage(makeSigned(MOCK_SIGNER_1, "ADA", tNow - 2000, 10));
    verifier.registerDataPackage(makeSigned(MOCK_SIGNER_2, "ADA", tNow - 2000, 11));

    const tested = makeSigned(UNAUTHORIZED_MOCK_SIGNER, "ADA", tNow, 10.4); // median=10.5, ~0.95%
    const result = verifier.verifyDataPackage(tested);
    expect(result).toBe(tested);
  });
  ...
  it("normalizes timestamps to 1s granulation when registering and verifying", () => {
    ...
    verifier.registerDataPackage(makeSigned(MOCK_SIGNER_1, "DOT", 1000_000_3001, 20));
    // Second ref at different second (4001ms -> 4000ms)
    verifier.registerDataPackage(makeSigned(MOCK_SIGNER_2, "DOT", 1000_000_4001, 22));

    // Test in ~3000ms -> only first ref -> not enough
    const tested1 = makeSigned(UNAUTHORIZED_MOCK_SIGNER, "DOT", 1000_000_3000, 21);
    ...
    verifier.registerDataPackage(makeSigned(MOCK_SIGNER_2, "DOT", 1000_000_3005, 22));
    const tested2 = makeSigned(UNAUTHORIZED_MOCK_SIGNER, "DOT", 1000_000_3002, 21);
